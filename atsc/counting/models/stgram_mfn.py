...
class Wave2Mel(object):
    def __init__(self, sr,
        ...
        self.mel_transform = torchaudio.transforms.MelSpectrogram(sample_rate=sr,
                                                                  ...
                                                                  n_mels=n_mels,
                                                                  power=power).to("cuda")
        self.amplitude_to_db = torchaudio.transforms.AmplitudeToDB(stype='power').to("cuda")

    def __call__(self, x):
        # spec =  self.amplitude_to_db(self.mel_transform(x)).squeeze().transpose(-1,-2)
        return self.amplitude_to_db(self.mel_transform(x).to("cuda"))
...
class MobileFaceNet(nn.Module):
    def __init__(self,
        ...
        # 20(10), 4(2), 8(4)
        #self.linear7 = ConvBlock(512, 512, (4, 135), 1, 0, dw=True, linear=True)
        self.linear7 = ConvBlock(512, 512, (2, 10), 1, 0, dw=True, linear=True)
        self.linear1 = ConvBlock(512, 128, 1, 1, 0, linear=True)

        self.fc_out = nn.Linear(6400, num_class)
        # init
...
if __name__ == '__main__':
    net = STgramMFN(num_classes=4)
    x_wav = torch.randn((6,4, 16000*60))
    x_mel = torch.randn((6, 4, 64, 1876))
    labels= torch.randn((6,1))
    print(x_wav[:, 0, :].shape)
    a,b=net(x_wav[:,0,:],x_mel[:,0,:,:],label=None)
    a1, b1 = net(x_wav[:, 1, :], x_mel[:, 1, :, :], label=None)
    IPython.embed()
    exit()
